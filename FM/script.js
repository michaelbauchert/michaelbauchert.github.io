...
function pointerPrototype () {//MB Mod: add fm synth to the pointer
    this.id = -1;
    this.x = 0;
    this.y = 0;
    this.dx = 0;
    ...
    this.moved = false;
    this.color = [30, 0, 300];
    this.env = new Tone.AmplitudeEnvelope (0.1 , 0.1 , 0.8 , 0.5);
    this.oscX = new Tone.Oscillator (0 , 'sine').start();
    this.oscY = new Tone.Oscillator (0 , 'sine').start().connect(this.env);
    this.fmodXY = new Tone.Multiply().connect(this.oscY.frequency);
...
canvas.addEventListener('touchstart', e => {
    ...
    for (let i = 0; i < touches.length; i++) {
        if (i >= pointers.length) {
            ...
            prevPointer.freqX.connect(newPointer.fmodLink, 0, 0);
        }
